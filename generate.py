...
def Generate_Body(torso_coordinates, link_size):
	...
	pyrosim.Send_Joint( name = "Torso_BackLeg" , parent= "Torso" , child = "BackLeg" , type = "revolute", position = joint_torso_backleg_coordinates, jointAxis = "0 1 0")
	pyrosim.Send_Cube(name="BackLeg", pos=backLeg_coordinates , size=link_size)
	pyrosim.Send_Joint( name = "Torso_FrontLeg" , parent= "Torso" , child = "FrontLeg" , type = "revolute", position = joint_torso_frontleg_coordinates, jointAxis = "0 1 0")
...
def Generate_Mind(link_size):
	...
	# pyrosim.Send_Synapse( sourceNeuronName = 2 , targetNeuronName = 3 , weight = random.random[0,1]  )
	# pyrosim.Send_Synapse( sourceNeuronName = 1 , targetNeuronName = 4 , weight = random.random[0,1]  )
	# pyrosim.Send_Synapse( sourceNeuronName = 2 , targetNeuronName = 4 , weight = random.random[0,1]  )
	for i in range(3):
		for j in range(3,5):
